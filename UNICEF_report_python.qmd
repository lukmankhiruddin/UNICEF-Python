...
fig.update_layout(
    title={
        'text': 'Top 5 Countries with Highest HIV Knowledge',
        'x': 0.5,
        'xanchor': 'center'
    },
...
fig.update_layout(
    title={
        'text': 'Bottom 5 Countries with Lowest HIV Knowledge',
        'x': 0.5,
        'xanchor': 'center'
    },
    xaxis_title='Percentage (%)',
...
fig = make_subplots(
    rows=2, 
    cols=2, 
    subplot_titles=top_trend_countries,
    vertical_spacing=0.2,  # Increase vertical spacing between plots
    horizontal_spacing=0.15  # Increase horizontal spacing between plots
)
    ...
    for sex, color in zip(['Female', 'Male'], [COLORS['red'], COLORS['blue']]):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate='Year: %{x}<br>' + sex + ': %{y:.1f}%<extra></extra>'
            ),
            row=row, col=col
        )
...
fig.update_layout(
    title={
        'text': 'Global Gender Gap in HIV Knowledge',
        'x': 0.5,
        'xanchor': 'center'
    },
...
# Create list of features for hover text
hover_text = []
for idx, row in africa.iterrows():
    hover_text.append(f"Country: {row['ADMIN']}<br>" +
                     f"Stunting Rate: {row['obs_value']:.1f}%" if pd.notna(row['obs_value']) 
                     else f"Country: {row['ADMIN']}<br>No data available")
...
fig.update_layout(
    title={
        'text': 'Child Stunting Burden in Africa',
        'x': 0.5,
        'xanchor': 'center'
    },
    geo=dict(
...
# Define Asian countries we want to include
asian_countries = [
...
asia = world[world['ADMIN'].isin(asian_countries)].copy()
...
# Get the most recent stunting data for each country
stunting_data = combined_indicators[
...
# Map country names
recent_stunting['country'] = recent_stunting['country'].replace(country_name_mapping)

# Merge with map data
asia = asia.merge(
    recent_stunting[['country', 'obs_value']],
    left_on='ADMIN',
    right_on='country',
    how='left'
)
...
fig.add_trace(go.Choropleth(
    geojson=json.loads(asia.to_json()),
    locations=asia.index,
    z=asia['obs_value'],
    colorscale='YlOrRd',
    colorbar_title='Stunting Rate (%)',
    hovertemplate='<b>%{text}</b><br>Stunting Rate: %{z:.1f}%<extra></extra>',
    text=asia['ADMIN'],
))
...
fig.update_geos(
    ...
    coastlinecolor='gray',
    fitbounds='locations',
    center=dict(lon=90, lat=30),  # Center on Asia
    resolution=110
)
...
fig.update_layout(
    title={
        'text': 'Child Stunting Burden in Asia',
        'x': 0.5,
        'xanchor': 'center'
    },
    height=600,
...
fig.update_layout(
    title={
        'text': 'Progress in Reducing Child Stunting in Bangladesh',
        'x': 0.5,
        'xanchor': 'center'
    },
    xaxis_title='Year',
...
fig.add_trace(go.Scatter(
    ...
    showlegend=False,
    hovertemplate="Country: %{text}<br>Female: %{x:.1f}%<br>Male: %{y:.1f}%<extra></extra>"
))
...
fig.update_layout(
    title={
        'text': 'HIV Knowledge: Male vs. Female Comparison',
        'x': 0.5,
        'xanchor': 'center'
    },
